...
def test_post_list_nested_no_join(client, register_routes, computer):
    payload = {
        "data": {
            ...
            "attributes": {
                "name": "test_name",
                "address": {"street": "test_street", "city": "test_city", "state": "NC", "zip": "00000"},
            },
        }
    }
    with client:
        response = client.post(
            "/string_json_attribute_persons", data=json.dumps(payload), content_type="application/vnd.api+json"
        )
...
def test_post_list_nested(client, register_routes, computer):
    payload = {
        "data": {
            "type": "person",
            "attributes": {"name": "test", "tags": [{"key": "k1", "value": "v1"}, {"key": "k2", "value": "v2"}]},
            "relationships": {"computers": {"data": [{"type": "computer", "id": str(computer.id)}]}},
        }
    }
...
def test_post_list_single(client, register_routes, person):
    payload = {
        "data": {
            "type": "computer",
            "attributes": {"serial": "1"},
            "relationships": {"owner": {"data": {"type": "person", "id": str(person.person_id)}}},
        }
    }
...
def test_get_relationship(session, client, register_routes, computer, person):
    ...
    with client:
        response = client.get(
            "/persons/" + str(person.person_id) + "/relationships/computers?include=computers",
            content_type="application/vnd.api+json",
        )
        assert response.status_code == 200
...
def test_get_relationship_single(session, client, register_routes, computer, person):
    ...
    with client:
        response = client.get(
            "/computers/" + str(computer.id) + "/relationships/owner", content_type="application/vnd.api+json"
        )
        assert response.status_code == 200
...
def test_issue_49(session, client, register_routes, person, person_2):
    with client:
        for p in [person, person_2]:
            response = client.get(
                "/persons/" + str(p.person_id) + "/relationships/computers?include=computers",
                content_type="application/vnd.api+json",
            )
            assert response.status_code == 200
...
def test_single_accept_header(client, register_routes):
    with client:
        response = client.get(
            "/persons", content_type="application/vnd.api+json", headers={"Accept": "application/vnd.api+json"}
        )
        assert response.status_code == 200
...
def test_get_relationship_relationship_field_not_found(client, register_routes, person):
    with client:
        response = client.get(
            "/persons/" + str(person.person_id) + "/relationships/computer", content_type="application/vnd.api+json"
        )
        assert response.status_code == 500
...
def test_get_list_name(client, register_routes):
    with client:
        querystring = urlencode(
            {"filter": json.dumps([{"name": f"computers{SPLIT_REL}serial", "op": "ilike", "val": "%1%"}])}
        )
        response = client.get("/persons" + "?" + querystring, content_type="application/vnd.api+json")
        assert response.status_code == 200
...
def test_get_list_no_op(client, register_routes):
    with client:
        querystring = urlencode({"filter": json.dumps([{"name": f"computers{SPLIT_REL}serial", "val": "1"}])})
        response = client.get("/persons" + "?" + querystring, content_type="application/vnd.api+json")
        assert response.status_code == 500
